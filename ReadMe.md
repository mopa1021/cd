...
> ❗ Vor dem 1. Start von PostgreSQL muss man die Skripte `create-db-cd.sql`
> und `create-schema-cd.sql` aus dem Verzeichnis `.extras\db\postgres\sql`
...
> nach `C:\Zimmermann\volumes\postgres\csv\cd`.

> ❗ Vor dem 1. Start von MySQL muss man das Skript `create-db-cd.sql` aus
...
> dem Verzeichnis `.extras\db\mysql\csv` nach `C:\Zimmermann\volumes\mysql\csv\cd`.
...
`db.populate` in `src\config\resources\cd.yml` wird festgelegt, ob die
(Test-) DB `cd` neu geladen wird.
...
- Im Eingabefeld _Name_ `cd` und im Eingabefeld _Summary_ z.B.
...
und wählt aus dem Verzeichnis `.extras\postman` die Datei `cd.postman_environment.json`
aus. Jetzt hat man die Umgebung `cd` mit der Variablen `base_url` und dem
...
```powershell
    npm exec jest --detectOpenHandles --errorOnDeprecated `
      --forceExit --runTestsByPath '__tests__\cd\cd-GET.controller.test.ts'
```
...
```powershell
    ...
    docker build --sbom true --tag patrickmoser/cd:2023.10.0-distroless .

    # Debian Bookworm (12) slim
    Get-Content Dockerfile.debian | docker run --rm --interactive hadolint/hadolint:2.12.1-beta-debian
    docker build --sbom true --tag patrickmoser/cd:2023.10.0-bookworm --file Dockerfile.bookworm .

    # Wolfi
    Get-Content Dockerfile.wolfi | docker run --rm --interactive hadolint/hadolint:2.12.1-beta-debian
    docker build --sbom true --tag patrickmoser/cd:2023.10.0-wolfi --file Dockerfile.wolfi .
```
...
```powershell
    docker inspect patrickmoser/cd:2023.10.0-distroless
    docker inspect patrickmoser/cd:2023.10.0-bookworm
    docker inspect patrickmoser/cd:2023.10.0-wolfi
```
...
```powershell
    docker sbom patrickmoser/cd:2023.10.0-distroless
    docker sbom patrickmoser/cd:2023.10.0-bookworm
    docker sbom patrickmoser/cd:2023.10.0-wolfi
```
...
```powershell
    ...
    # PowerShell fuer cd-Server mit distroless-Image zzgl. DB-Server und Mailserver
    docker compose up

    # alternativ: cd-Image mit Debian Bookworm
    docker compose -f compose.yml -f compose.bookworm.yml up

    # alternativ: cd-Image mit Wolfi
    ...
    # 2. Powershell: cd-Server einschl. DB-Server und Mailserver herunterfahren
    cd .extras/compose
    docker compose down
```
...
```powershell
    docker scout quickview patrickmoser/cd:2023.10.0-distroless
    docker scout quickview patrickmoser/cd:2023.10.0-bookworm
    docker scout quickview patrickmoser/cd:2023.10.0-wolfi
```
...
```powershell
    docker scout cves patrickmoser/cd:2023.10.0-bookworm
    docker scout cves --format only-packages patrickmoser/cd:2023.10.0-bookworm
````
...
Service für Trivy so konfiguriert, dass das Image `cd` analysiert wird.
...
```powershell
    ...
    docker build --sbom true --tag patrickmoser/jenkins:2023.10.0 .
```

Das neu gebaute Image `patrickmoser/jenkins:2023.10.0` wird in
...
- "Feature Filenames", z.B. cd.service.ts
